...
function Explorer() {
  const [data] = useState<SwarmContext>(MOCK);
  ...
  const completed = data.tasks.filter(t => t.status === 'complete').length;
  const avgConf = data.facts.length > 0 ? data.facts.reduce((s, f) => s + f.confidence, 0) / data.facts.length : 0;
    ...
    <section id="explorer" style={{ borderTop: '1px solid #1E2D4A', backgroundColor: '#0D1428' }}>
      <div style={{ maxWidth: '1160px', margin: '0 auto', padding: '72px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '18px' }}>
          {[
            { label: 'Facts', value: data.facts.length, sub: `${data.facts.filter(f => f.reviewed).length} reviewed`, color: '#1D6FEB', icon: <Database size={15} color="#1D6FEB" /> },
            { label: 'Tasks', value: `${completed}/${data.tasks.length}`, sub: `${data.tasks.length - completed} remaining`, color: '#00D4AA', icon: <CheckCircle size={15} color="#00D4AA" /> },
            { label: 'Insights', value: data.insights.length, sub: 'from reflection', color: '#A855F7', icon: <Zap size={15} color="#A855F7" /> },
              ...
              {(['facts', 'tasks', 'insights'] as const).map((t) => (
                ...
                >
                  ...
                  <span style={{ marginLeft: '5px', padding: '1px 6px', borderRadius: '9px', fontSize: '10px', backgroundColor: tab === t ? '#1D6FEB20' : '#1E2D4A', color: tab === t ? '#1D6FEB' : '#8A9BB5' }}>
                    {t === 'facts' ? data.facts.length : t === 'tasks' ? data.tasks.length : data.insights.length}
                  </span>
                </button>
              ))}
            </div>
